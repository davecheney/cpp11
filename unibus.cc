...
void write16(uint32_t a, uint16_t v) {
  if (a % 1) {
    printf("unibus: write16 to odd address %06lo\n", a);
    trap(INTBUS);
  }
  if (a < 0760000) {
    unibus::intptr[a >> 1] = v;
  ...
  printf("unibus: write to invalid address %06lo\n", a);
  trap(INTBUS);
}

uint16_t read16(uint32_t a) {
  if (a & 1) {
    printf("unibus: read16 from odd address %06lo\n", a);
    trap(INTBUS);
  }
  if (a < 0760000) {
  ...
  printf("unibus: read from invalid address %06lo\n", a);
  return trap(INTBUS);
}
};
